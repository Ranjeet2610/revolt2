...
async function start_everything(IDENTIFIER_USER, IS_HEADLESS = IS_HEADLESS_OVERRIDE, START_IMMEDIATELY = true) {
  ...
  // }
    ...
    console.log({
      type: "DebugMessage",
      message: `Session for user "${IDENTIFIER_USER}" started`,
    });
  }
  // process.exit();
  ...
  for (const [key, file] of Object.entries(files)) {
    if (!fs.existsSync(`./${IDENTIFIER_USER}/${file}`)) {
      console.log(file);
      fs.writeFileSync(
        `./${IDENTIFIER_USER}/${file}`,
        JSON.stringify(initialValues[key], null, 2)
      );
      addLog({
        type: "DebugMessage",
        message: `Created ${file} with initial value`,
      });
    } else {
  ...
  var responses = JSON.parse(
    fs.readFileSync(`./${IDENTIFIER_USER}/responses.json`).toString()
  );
  var canReply = JSON.parse(
    fs.readFileSync(`./${IDENTIFIER_USER}/canreply.json`).toString()
  );
  var response_delay = JSON.parse(
    fs.readFileSync(`./${IDENTIFIER_USER}/response_delay.json`).toString()
  );
  var isBotOn = JSON.parse(
    fs.readFileSync(`./${IDENTIFIER_USER}/is_bot_on.json`).toString()
  );
  var responseType = JSON.parse(
    fs.readFileSync(`./${IDENTIFIER_USER}/response_type.json`).toString()
  );
  var instantResponses = JSON.parse(
    fs.readFileSync(`./${IDENTIFIER_USER}/instant_responses.json`).toString()
  );
  var token = "";
  var error = 0;
  ...
  eventEmitter.on("Ready", (msg) => {
    const user =
      msg.users[msg.users.findIndex((user) => user.relationship == "User")];
    fs.writeFileSync(
      `./${IDENTIFIER_USER}/account_info.json`,
      JSON.stringify(user)
    );
    clientInfo = msg;
  ...
  eventEmitter.on("Message", async (msg, page) => {
    ...
    var channel_index = clientInfo.channels.findIndex(
      (obj) => obj._id == msg?.channel
    );
    var channel =
      channel_index != -1 ? clientInfo.channels[channel_index] : undefined;
    ...
    if (channel.channel_type == "DirectMessage") {
      return addLog({
        type: "DebugMessage",
        message: "Message is DirectMessage, skipping",
      });
    }

    var server_index = clientInfo.servers.findIndex(
      (obj) => obj._id == channel?.server
    );
    var server =
      server_index != -1 ? clientInfo.servers[server_index] : undefined;

    console.log({ server_index, server });

    var category;
    if (server?.categories) {
      var category_index = server?.categories?.findIndex((obj) =>
        obj.channels.includes(channel._id)
      );
      category =
        category_index != -1 ? server?.categories[category_index] : undefined;
    }

    console.log({ category_index, category });

    var canReply = await getCanReply(
    ...
    if (canReply.canReply) {
      var instantResponse = await getInstantResponse(
        msg?.content,
        server._id,
        channel?.name
      );
      ...
      if (instantResponse.found) {
        addLog({
          type: "BotMessage",
          message: `✅ I can reply here. Reason: ${canReply.reason} & instant response match`,
        });
        if (instantResponse?.response) {
          var result = await sendMessage(
            msg?.channel,
            instantResponse?.response?.respondWith,
            page
          );
          ...
          addLog({
            type: "DebugMessage",
            message:
              "Instant response gave no result; possible reason: no number on channel name to extract",
          });
        }
      }
    }
  ...
  eventEmitter.on("ChannelCreate", async (msg, page) => {
    ...
    if (!isBotOn.status) {
      return addLog({
        type: "DebugMessage",
        message: "Bot is currently set to OFF. Responses will not be sent.",
      });
    }
    ...
    if (
      fs
        .readFileSync(`./${IDENTIFIER_USER}/already_responded.txt`)
        .toString()
        .includes(msg._id)
    ) {
      addLog({
        type: "BotMessage",
        message: `⚠️ I already responded here. Skipping. ID: ${msg._id} | Name: ${msg.name}`,
      });
    } else {
      if (_canReply.canReply) {
        addLog({
          type: "BotMessage",
          message: `✅ I can reply here. ID: ${msg._id} | Name: ${msg.name} | Reason: ${_canReply.reason}`,
        });

        fs.appendFileSync(
          `./${IDENTIFIER_USER}/already_responded.txt`,
          msg._id + "\n"
        );
        var response = (await getReplyWith(msg.server))
          ? await getReplyWith(msg.server)
          : "";
        ...
        if (response) {
          addLog({
            type: "BotMessage",
            message: `Waiting for ${delay / 1000} seconds to send response`,
          });
          ...
          setTimeout(async () => {
            try {
              ...
              if (response) {
                ...
                addLog({
                  type: "BotMessage",
                  message: `Response successfully sent to "${msg.name}".`,
                });
                addLog({
                  type: "DebugMessage",
                ...
                addLog({
                  type: "BotMessage",
                  message: `No number was extracted from "${msg.name}".`,
                });
              }
            } catch (error) {
              console.log(error);
              addLog({
                type: "BotMessage",
                message: `Something went wrong when sending repsonse. ID: ${msg._id} | Name: ${msg.name}`,
              });
            }
          }, delay);
        } else {
          addLog({
            type: "BotMessage",
            message: `🤚 Set response is empty. I will not be replying. ID: ${msg._id} | Name: ${msg.name} | Reason: ${_canReply.reason}`,
          });
        }
      } else {
    ...
    for (let i = 0; i < clonedChannels.length; i++) {
      ...
      if (`${JSON.stringify(msg?.data?.categories)}`.includes(channel._id)) {
        if (
          ...
          addLog({
            type: "BotMessage",
            message: `⚠️ I already responded here. Skipping. ID: ${channel._id} | Name: ${channel.name}`,
          });
        } else {
          ...
          if (_canReply.canReply) {
            addLog({
              type: "BotMessage",
              message: `✅ I can reply here. ID: ${channel._id} | Name: ${channel.name} | Reason: ${_canReply.reason}`,
            });
            var response = (await getReplyWith(channel.server))
              ? await getReplyWith(channel.server)
              : "";

            if (response) {
              console.log(channel);

              var delay = getRandomInt(
                response_delay.min_ms,
                response_delay.max_ms
              );
              ...
              setTimeout(async () => {
                try {
                  ...
                  if (response) {
                    ...
                    addLog({
                      type: "BotMessage",
                      message: `Response successfully sent to "${channel.name}".`,
                    });
                    addLog({
                      type: "DebugMessage",
                      message: JSON.stringify(result),
                  ...
                  addLog({
                    type: "BotMessage",
                    message: `Something went wrong when sending repsonse. ID: ${channel._id} | Name: ${channel.name}`,
                  });
                }
              }, delay);
            } else {
            ...
            addLog({
              type: "BotMessage",
              message: `❌ I can't reply here. ID: ${channel._id} | Name: ${channel.name} | Reason: no keyword or category match`,
            });
          }
        }
      }
  ...
  eventEmitter.on("ServerMemberLeave", (msg) => {
    console.log(
      clientInfo.users[
        clientInfo?.users.findIndex((user) => user.relationship == "User")
      ]._id
    );
    if (
      clientInfo.users[
  ...
  eventEmitter.on("Debug", (msg) => {
    ...
    if (msg.includes("Closed with reason:")) {
      ...
      if (error >= 20) {
        ...
        return addLog({
          type: "FatalError",
          message: "Too much close just occured. Consider logging in again.",
        });
      }
  ...
  async function start() {
    ...
    if (isBotOn.status) {
      try {
        ...
        addLog({
          type: "DebugMessage",
          message: "Trying to open Puppeteer browser",
        });
      ...
      addLog({
        type: "BotStatus",
        message:
          "Bot is currently set to OFF. No information will be sent from server to client.",
      });
  ...
  async function initialize_puppeteer() {
    ...
    if (browser_restart_count >= max_restarts || restart_disabled) {
      console.log(
        "❌ Maximum browser restarts reached or restarts disabled. Stopping to prevent infinite loop."
      );
      addLog({
        type: "FatalError",
        message:
          "Maximum browser restarts reached. Please check your Revolt login or restart the application.",
      });
      return;
    }
    ...
    const launch_mode = force_headful
      ? "headful"
      : IS_HEADLESS
      ? "headless"
      : "headful";
    console.log(
      `🔧 Launching Chrome in ${launch_mode} mode (restart #${browser_restart_count})`
    );
    ...
    if (!executablePath) {
      throw new Error(
        "No Chrome/Chromium executable found. Please install Chromium or Google Chrome."
      );
    }

    console.log(`🔧 Using browser: ${executablePath}`);
    console.log(
      `🔧 Browser settings: force_headful=${force_headful}, IS_HEADLESS=${IS_HEADLESS}, final_mode=${
        force_headful ? "headful" : IS_HEADLESS ? "headless" : "headful"
      }`
    );
    ...
    auth_timeout = setTimeout(async () => {
      if (browser_restart_count >= max_restarts) {
        console.log(
          "⏰ Authentication timeout reached. Stopping browser to prevent infinite loop."
        );
        addLog({
          type: "FatalError",
          message:
            "Authentication timeout. Please check your Revolt login credentials.",
        });
        try {
          await browser.close();
    ...
    addLog({
      type: "DebugMessage",
      message:
        "Puppeteer browser has launched. Bot dashboard panel will open once Revolt account is authenticated.",
    });
    addLog({
      type: "DebugMessage",
      message: `Puppeteer browser is currently running in ${
        (force_headful ? false : IS_HEADLESS) ? "Headless" : "Headful"
      } mode`,
    });
          ...
          if (parsed.type == "Authenticate") {
            ...
            if (force_headful) {
              addLog({
                type: "DebugMessage",
                message:
                  "Successfully authenticated. Restarting Puppeteer in headless mode",
              });
              force_headful = false;
    ...
    page.on("framenavigated", async (frame) => {
      if (
        frame.url().startsWith("https://revolt.onech.at/login") &&
        !force_headful &&
        !login_restart_done &&
        browser_restart_count < max_restarts &&
        !restart_disabled
      ) {
        force_headful = true;
        ...
        addLog({
          type: "DebugMessage",
          message: `Revolt redirected to "/login"`,
        });
        addLog({
          type: "DebugMessage",
          message: `Restarting Puppeteer browser in headful mode to show log in prompt (LAST RESTART)`,
        });
        ...
        addLog({
          type: "DebugMessage",
          message: `Please log in to Revolt in the browser window that opened. The bot will continue once authenticated.`,
        });
        addLog({
          type: "DebugMessage",
          message: `If login fails, please restart the application.`,
        });
      ...
      if (
        ((await page.content())
          .toLowerCase()
          .includes("security of your connection") ||
          (await page.content()).toLowerCase().includes("blocked")) &&
        !force_headful &&
        !login_restart_done &&
        browser_restart_count < max_restarts &&
        !restart_disabled
        ...
        addLog({
          type: "DebugMessage",
          message: `Cloudflare detected. Restarting in headful mode`,
        });
        force_headful = true;
        login_restart_done = true; // Prevent multiple restarts
        restart_disabled = true; // Disable further restarts
        ...
        addLog({
          type: "DebugMessage",
          message: `Cloudflare detected. Please complete the security check in the browser window.`,
        });
        addLog({
          type: "DebugMessage",
          message: `The bot will continue once the security check is completed.`,
        });
  ...
  async function addInstantResponse(
    serverId = "",
    message = "",
    respondWith = "",
    regex = false,
    ...
    fs.writeFileSync(
      `./${IDENTIFIER_USER}/instant_responses.json`,
      JSON.stringify(instantResponses)
    );
  }
  ...
  async function setReplyWith(string, serverId) {
    ...
    fs.writeFileSync(
      `./${IDENTIFIER_USER}/responses.json`,
  ...
  async function addReplyWithKeyword(string, serverId) {
    ...
    if (
      responses[serverId + "_keywords_is_case_sensitive"] != true &&
    ...
    // console.log({ serverId, string });
    responses[serverId + "_keywords"] = [
      ...new Set(responses[serverId + "_keywords"]),
    ];
    clientInfo.responses = responses;
    clientInfo.canReply = canReply;
    console.log(responses);
  ...
  async function setCanReply(categoryId) {
    ...
    fs.writeFileSync(
      `./${IDENTIFIER_USER}/canreply.json`,
      JSON.stringify([...new Set(canReply)])
    );
  }
  ...
  async function getInstantResponse(message, serverId, channelName) {
    ...
    if (message && channelName) {
      for (
        ...
        index <
        Object.values(
          instantResponses[serverId] ? instantResponses[serverId] : {}
        ).length;
        index++
      ) {
  ...
  async function getCanReply(categoryId, channelName, serverId) {
    if (responses[`${serverId}_keywords`]) {
      ...
      for (
        let index = 0;
        index < responses[`${serverId}_keywords`].length;
        index++
      ) {
  ...
  async function setBotStatus(status) {
    if (status == true || status == false) {
      ...
      fs.writeFileSync(
        `./${IDENTIFIER_USER}/is_bot_on.json`,
        JSON.stringify(isBotOn)
      );
      ...
      if (!status) {
        ...
        addLog({
          type: "DebugMessage",
          message: `Puppeteer browser has been closed`,
        });
      } else {
        initialize_puppeteer();
      }
    ...
    return await page.evaluate(
      async (token, serverId, leaveSilently) => {
        var result = await fetch(
          `https://revolt-api.onech.at/servers/${serverId}/${
            leaveSilently ? "?leave_silently=true" : "?leave_silently=false"
          }`,
          {
            method: "DELETE",
            headers: {
  ...
  async function setResponseDelay(min, max) {
    ...
    fs.writeFileSync(
      `./${IDENTIFIER_USER}/response_delay.json`,
      JSON.stringify(response_delay)
    );
  ...
  async function setResponseType(type, server_id) {
    ...
    fs.writeFileSync(
      `./${IDENTIFIER_USER}/response_type.json`,
  ...
  async function sendMessage(id, content, page) {
    ...
    return await page.evaluate(
      async (id, content, token) => {
        var result = await fetch(
          `https://revolt-api.onech.at/channels/${id}/messages`,
          {
            method: "POST",
            headers: {
              "X-Session-Token": token,
  ...
  app.post("/api/set_response", async (req, res) => {
    if (!req.query.response) {
      ...
      addLog({
        type: "DebugMessage",
        message: `Bot response set to empty on server "${req.query.serverId}"`,
      });
    ...
    if (!req.query.serverId) {
      return res
        .status(400)
        .json({ error: true, message: "Server ID is empty" });
    }
    ...
    addLog({
      type: "DebugMessage",
      message: `Bot will now respond "${req.query.response}" on server "${req.query.serverId}"`,
    });
  ...
  app.post("/api/set_response_keyword_case_sensitive", async (req, res) => {
    ...
    if (!req.query.state != true && !req.query.state != false) {
      return res
        .status(400)
        .json({ error: true, message: "State is not a boolean" });
    }
    if (!req.query.serverId) {
      return res
        .status(400)
    ...
    addLog({
      type: "DebugMessage",
      message: `Keyword responding is now ${
        req.query.state ? "case sensitive" : "case insensitive"
      } on "${req.query.serverId}"`,
    });
  ...
  app.post("/api/add_response_keyword", async (req, res) => {
    ...
    try {
      var result = await addReplyWithKeyword(
        req.query.string,
        req.query.serverId
      );
      ...
      addLog({
        type: "DebugMessage",
        message: `Bot will now respond to categories on server "${req.query.serverId}" with the keyword ${req.query.string}`,
      });
  ...
  app.post("/api/delete_keyword", async (req, res) => {
    ...
    try {
      ...
      addLog({
        type: "DebugMessage",
        message: `Keyword "${req.query.string}" for matching has been removed on server ${req.query.serverId}`,
      });
  ...
  app.post("/api/set_can_reply", async (req, res) => {
    if (!req.query.categoryId) {
      return res
        .status(400)
        .json({ error: true, message: "Category ID is empty" });
    }

    addLog({
      type: "DebugMessage",
      message: `Bot will now respond on category "${req.query.categoryId}"`,
    });
  ...
  app.post("/api/join_server", async (req, res) => {
    if (!req.query.serverUrl) {
      return res
        .status(400)
        .json({ error: true, message: "Server URL is empty" });
    }
    try {
      const result = await joinServer(req.query.serverUrl, global_page);

      if (
        !clientInfo.servers.some((server) => server._id == result.server._id)
      ) {
      ...
      if (result.error) {
        res
          ...
          .json({
            error: true,
            message: `Something went wrong in joining the server link.`,
        ...
        addLog({
          type: "DebugMessage",
          message: `Bot has joined the server with the invite link "${req.query.serverUrl}"`,
        });
    ...
    try {
      ...
      if (result.error) {
        res
          ...
          .json({
            error: true,
            message: `Something went wrong in leaving server.`,
          });
      } else {
        addLog({
          type: "DebugMessage",
          message: `Bot has left the server "${req.query.serverId}"`,
        });
        res.json({ ...result, clientInfo });
      }
    } catch (error) {
  ...
  app.delete("/api/set_can_reply", async (req, res) => {
    ...
    addLog({
      type: "DebugMessage",
      message: `Bot will now stop responding on category "${req.query.categoryId}"`,
    });
  ...
  app.post("/api/set_bot_status", async (req, res) => {
    try {
      if (!req.query.status) {
        return res
          .status(400)
          .json({ error: true, message: "Status is empty" });
      }
      ...
      addLog({
        type: "BotStatus",
        message: isBotOn.status
          ? "Bot is now set to: ON"
          : "Bot is now set to: OFF",
      });
      res
        .json({
          error: false,
          message: `Bot is now turned ${isBotOn.status ? "ON" : "OFF"}`,
        })
        .status(error.status);
    } catch (error) {
      console.log(error);
      addLog({
        type: "BotStatus",
        message: "Something went wrong when setting bot status.",
      });
      res
        .json({
          error: true,
  ...
  app.post("/api/set_response_delay", async (req, res) => {
    try {
      if (!req.query.min) {
        return res
          .status(400)
          .json({ error: true, message: "Minimum is empty" });
      }
      if (!req.query.max) {
        return res
          .status(400)
          .json({ error: true, message: "Maximum is empty" });
      }

      var result = await setResponseDelay(req.query.min, req.query.max);
      addLog({
        type: "DebugMessage",
        message: `Response delay successfully set, bot will now pick a random number from ${req.query.min} to ${req.query.max} as millisecond value delay`,
      });
      ...
      res
        ...
        .json({
          error: true,
          message: "Something went wrong when setting response delay.",
        });
    }
  });
    ...
    try {
      if (!response_types.includes(req.query.response_type)) {
        return res
          ...
          .json({
            error: true,
            message: `Response type "${req.query.response_type}" is not valid.`,
          });
      }
      ...
      var result = await setResponseType(
        req.query.response_type,
        req.query.serverId
      );
      addLog({
        type: "DebugMessage",
        message: `Response type successfully set to ${
          req.query.response_type
        }: ${response_type_definition[req.query.response_type]} in server ${
          req.query.serverId
        }`,
      });
      ...
      res
        ...
        .json({
          error: true,
          message: "Something went wrong when setting response type.",
        });
    }
  });
  ...
  app.post("/api/instant_response", async (req, res) => {
    try {
      const { serverId, message, respondWith, regex, caseSensitive, uuid } =
        req.query;
      ...
      if (!message) {
        return res
          .status(400)
          .json({ error: true, message: "Message is empty" });
      }
      ...
      if (!respondWith && !regex) {
        return res
          .status(400)
          .json({ error: true, message: "Response is empty" });
      }
      ...
      if (!regex) {
        return res
          .status(400)
          .json({ error: true, message: "Response type is empty" });
      }
      ...
      if (caseSensitive && !["true", "false"].includes(caseSensitive)) {
        return res
          ...
          .json({
            error: true,
            message: "caseSensitive must be 'true' or 'false'",
          });
      }

      var result = await addInstantResponse(
        serverId,
        message,
        respondWith,
        regex,
      ...
      addLog({
        type: "DebugMessage",
        message: `Instant response added in server ${serverId}`,
      });
      res.json({
        error: false,
        message: "Successfully added instant response.",
      });
      io.emit("instant_responses", clientInfo.instantResponses);
    } catch (error) {
      res.json({
        error: true,
        message: "Something went wrong when adding instant response.",
      });
    }
  });
  ...
  app.delete("/api/instant_response", async (req, res) => {
    try {
      ...
      addLog({
        type: "DebugMessage",
        message: `Instant response deleted in server ${serverId}`,
      });
      res.json({
        error: false,
        message: "Successfully deleted instant response.",
      });
      io.emit("instant_responses", clientInfo.instantResponses);
    } catch (error) {
      console.log(error);
      res.json({
        error: true,
        message: "Something went wrong when deleting instant response.",
      });
    }
  });
  ...
  try {
    addLog({
      type: "DebugMessage",
      message: "Trying to start bot dashboard server",
    });
    ...
    server.listen(port, () => {
      ...
      addLog({
        type: "DebugMessage",
        message: `Now listening to: http://localhost:${port}`,
      });
    });
  } catch (error) {
    if (error.code == "ERR_SERVER_ALREADY_LISTEN") {
      addLog({
        type: "DebugMessage",
        message: "Bot dashboard server already running",
      });
    }
