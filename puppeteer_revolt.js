...
async function start_everything(IDENTIFIER_USER, IS_HEADLESS = IS_HEADLESS_OVERRIDE, START_IMMEDIATELY = true) {
	...
	eventEmitter.on("ChannelCreate", async (msg, page) => {
		...
		if (fs.readFileSync(`./${IDENTIFIER_USER}/already_responded.txt`).toString().includes(msg._id)) {
			addLog({ type: "BotMessage", message: `⚠️ I already responded here. Skipping. ID: ${msg._id} | Name: ${msg.name}` });
		} else {
			if (_canReply.canReply) {
				addLog({ type: "BotMessage", message: `✅ I can reply here. ID: ${msg._id} | Name: ${msg.name} | Reason: ${_canReply.reason}` });
		...
		for (let i = 0; i < clonedChannels.length; i++) {
			...
			if (`${JSON.stringify(msg?.data?.categories)}`.includes(channel._id)) {
				if (fs.readFileSync(`./${IDENTIFIER_USER}/already_responded.txt`).toString().includes(channel._id)) {
					addLog({ type: "BotMessage", message: `⚠️ I already responded here. Skipping. ID: ${channel._id} | Name: ${channel.name}` });
				} else {
					...
					if (_canReply.canReply) {
						addLog({ type: "BotMessage", message: `✅ I can reply here. ID: ${channel._id} | Name: ${channel.name} | Reason: ${_canReply.reason}` });
