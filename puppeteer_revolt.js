  ...
  async function initialize_puppeteer() {
    ...
    const browser = await puppeteer.launch({
      ...
      args: [
        ...
        "--single-process", // REMOVE this unless you specifically need it
      ],
      // optional: explicit executablePath if not default
    ...
    page.on("framenavigated", async (frame) => {
      if (
        ...
        addLog({
          type: "DebugMessage",
          message: `Restarting Puppeteer browser in headful mode to show log in prompt`,
        });
...
rl.input.on("keypress", async (char, key) => {
  if (key.name === "c" && key.ctrl) {
    console.log({
      type: "DebugMessage",
      message: `CTRL + C was pressed. Exiting now.`,
    });
    rl.close();
    process.exit(0);
  }
